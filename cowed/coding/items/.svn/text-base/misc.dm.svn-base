			...
			Plant()
				...
				for(var/turf/T in range(0))
					if(T.icon_state=="tilled soil")
						...
						return
		Wheat_Seeds
			GrowType=/item/misc/food/Wheat
			Length=250
		Corn_Seeds
			GrowType=/item/misc/food/Corn
			Length=200
		Tomato_Seeds
			GrowType=/item/misc/food/Tomato
			Length=150
		Potato_Seeds
			GrowType=/item/misc/food/Potato
			Length=100
		Watermelon_Seeds
			GrowType=/item/misc/food/Watermelon
			Length=220
		Carrot_Seeds
			GrowType=/item/misc/food/Carrot
			Length=120
		Apple_Seeds
...
/*	berrys
	...
	molten_copper
		...
		stacked = 1
		MouseDrop(obj/over_object,src_location,over_location,src_control,over_control,params)
			if(!(src in usr.contents) || !(over_object in usr.contents)) return ..()
			if(istype(over_object,/item/misc/key_mold))
