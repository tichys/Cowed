	...
	main
		...
		DisplayHUD(mob/M)
			...
			for(var/i = 1 to 15)
				...
				C.screen += new/screen/number(i, 11) //hunger
		...
		UpdateHUD(mob/M)
			...
			M.hud_tabHealth.icon_state = "[max(0, min(M.HP, 100))]"
...
screen //screen object: goes in client.screen. heh.
	...
	buttons
		...
		help
			...
			screen_loc = "14,1"
			Click() spawn if(usr && usr.client)
				usr << browse({"<body onload="parent.location='http://www.cowed.org/manual';">"}, "window=manual;size=750x500")
		reset_reboot_timer
			icon_state = "rtimer0"
			screen_loc = "8,17:16"
			Click()
