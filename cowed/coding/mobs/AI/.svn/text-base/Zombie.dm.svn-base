	...
	Loop()
		while(src && HP>0)
			...
			for(var/mob/M in view(20,src))
				...
				if(M != src && !istype(M,/mob/AI/Zombie) && M.name!="Zombie"&& !M.isMonster && !N && !findtext(M.name,"corpse"))
					target = M
					break
			...
			if(!target)
				for(var/mob/M in range(10,src))
					var/item/misc/books/Necro/N = locate() in M
					if(prob(40) && M != src && !istype(M,/mob/AI/Zombie) && M.name!="Zombie"&& !M.isMonster && !N && !findtext(M.name,"corpse"))
						target = M
						break
