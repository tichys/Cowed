...
mob
	...
	Login()
		...
		if(!loggedin)
			...
			send_message(usr, "\red <b>Full permission by Savaril for his icons!", 3)
	...
	Stat()
		if(statpanel("Stats"))
			...
			if(skills)
				stat("")
				stat("<b>Skills</b>")
				stat("Carpenting", "[skills.carpenting]/85")
				//stat("Hunting", "[skills.hunting]/85")
				stat("Gathering", "[skills.gathering]/85")
				stat("Fishing", "[skills.fishing]/85")
				stat("Mining", "[skills.mining]/85")
				//stat("Recycling", "[skills.recycling]/85")
				stat("Looting", "[skills.looting]/60")
			stat("")
...
/*		if(usr.passworded == 1)
	...
	/*	berry_effect(state)
						...
						if(M.spell_shield && (get_dir(M, src) & M.dir))
							...
							if(M.SLEEP <= 90 && prob(25))
								if(prob(50))
									...
									for(var/mob/N in hearers(M))
										N.show_message("\red <b>[M.name] is weakened!</b>")
							else M.SLEEP -= 10
							M.last_hurt = "blunt"
							hud_main.UpdateHUD(M)
							M.checkdead(M)
		...
		stun(time)
			stunned = max(stunned, time)
		weaken(time)
			weakened = max(weakened, time)
			for(var/spell/S in spells) S.onSleep(src)
			icon_state = "dead"
		checkdead(mob/M)
